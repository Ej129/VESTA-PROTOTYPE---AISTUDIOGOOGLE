...
import { Workspace, WorkspaceMember, AuditLog, AuditLogAction } from '../../src/types';
...
export const handler: Handler = async (event, context) => {
    try {
        ...
        const { workspaceId, status } = JSON.parse(event.body);
        if (!workspaceId || !status || !['active', 'archived'].includes(status)) {
        ...
        if (!currentUser || currentUser.role !== "Administrator") {
            return { statusCode: 403, body: JSON.stringify({ error: "Forbidden: Only administrators can perform this action." }), headers: { "Content-Type": "application/json" } };
        }
        ...
        const action: AuditLogAction = status === 'archived' ? 'Workspace Archived' : 'Workspace Unarchived';
        const newLog: AuditLog = {
            id: `log-${Date.now()}`,
            timestamp: new Date().toISOString(),
            userEmail: user.email,
            action,
            details: `Workspace "${workspace.name}" status changed to ${status}.`,
        };
        logs.unshift(newLog);
        ...
        console.error(`Error in update-workspace-status: ${errorMessage}`, error);
