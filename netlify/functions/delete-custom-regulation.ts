...
export const handler: Handler = async (event, context) => {
    try {
        ...
        const { workspaceId, regulationId } = JSON.parse(event.body);
        if (!workspaceId || !regulationId) {
        ...
        const updatedRegulations = regulations.filter(r => r.id !== regulationId);
        
        await store.setJSON(workspaceId, updatedRegulations);

        return { statusCode: 204, body: '' };

    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : "An unknown error occurred.";
        console.error(`Error in delete-custom-regulation: ${errorMessage}`, error);
