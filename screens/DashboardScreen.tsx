...
const AnalysisCard: React.FC<AnalysisCardProps> = ({ report, action, userRole, onArchive, onUnarchive, onDelete }) => {
    ...
    return (
        <div 
            className="bg-light-card dark:bg-dark-card p-5 rounded-xl shadow-md hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between border border-border-light dark:border-border-dark relative"
        >
            {userRole === 'Administrator' && (
                <div ref={menuRef} className="absolute top-3 right-3 z-10">
                    <button 
                        onClick={(e) => { e.stopPropagation(); setMenuOpen(o => !o); }} 
                        className="p-2 rounded-full text-secondary-text-light dark:text-secondary-text-dark hover:bg-gray-200 dark:hover:bg-dark-main"
                        aria-label="More options"
                    >
                        <MoreVerticalIcon className="w-5 h-5" />
                    </button>
                    {menuOpen && (
                        <div className="absolute right-0 mt-2 w-40 bg-light-card dark:bg-dark-card rounded-md shadow-lg z-20 border border-border-light dark:border-border-dark py-1">
                            {isArchived ? (
                                <button onClick={(e) => { e.stopPropagation(); onUnarchive(); setMenuOpen(false); }} className="w-full text-left px-4 py-2 text-sm text-primary-text-light dark:text-primary-text-dark hover:bg-gray-100 dark:hover:bg-dark-main">Unarchive</button>
                            ) : (
                                <button onClick={(e) => { e.stopPropagation(); onArchive(); setMenuOpen(false); }} className="w-full text-left px-4 py-2 text-sm text-primary-text-light dark:text-primary-text-dark hover:bg-gray-100 dark:hover:bg-dark-main">Archive</button>
                            )}
                            <button onClick={(e) => { e.stopPropagation(); onDelete(); setMenuOpen(false); }} className="w-full text-left px-4 py-2 text-sm text-red-600 dark:text-red-500 hover:bg-gray-100 dark:hover:bg-dark-main">Delete</button>
                        </div>
                    )}
                </div>
            )}
            <div onClick={action} className="cursor-pointer">
                <h3 className="font-bold text-lg text-primary-text-light dark:text-primary-text-dark truncate pr-8">{report.title}</h3>
...
const ConfirmationModal: React.FC<{ title: string; message: string; onConfirm: () => void; onCancel: () => void; }> = ({ title, message, onConfirm, onCancel }) => {
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 animate-fade-in" aria-modal="true" role="dialog">
            <div className="bg-light-card dark:bg-dark-card rounded-xl shadow-2xl p-8 max-w-md w-full transform transition-all animate-fade-in-up">
                <h2 className="text-xl font-bold text-accent-critical mb-4">{title}</h2>
                <p className="text-secondary-text-light dark:text-secondary-text-dark mb-8">{message}</p>
                <div className="flex justify-end gap-4">
                    <button onClick={onCancel} className="px-6 py-2 font-bold rounded-lg hover:bg-gray-200 dark:hover:bg-dark-sidebar transition-all text-secondary-text-light dark:text-secondary-text-dark border border-transparent">Cancel</button>
                    <button onClick={onConfirm} className="bg-accent-critical text-white font-bold py-2 px-6 rounded-lg hover:bg-opacity-90 transition-all">Confirm Delete</button>
                </div>
            </div>
            <style>{`
...
const StatCard: React.FC<{ title: string; value: string | number; icon: React.ReactNode }> = ({ title, value, icon }) => (
  <div className="bg-light-card dark:bg-dark-card p-5 rounded-xl shadow-md border border-border-light dark:border-border-dark flex items-center space-x-4">
...
const AtAGlance: React.FC<{reports: AnalysisReport[]}> = ({ reports }) => {
    ...
    return (
        <div className="bg-light-card dark:bg-dark-card p-6 rounded-xl shadow-md border border-border-light dark:border-border-dark">
          ...
          <div className="space-y-8">
              <div className="flex justify-between items-center">
                  <div>
                      ...
                      <p className="text-secondary-text-light dark:text-secondary-text-dark">Welcome back, {currentUser.name}!</p>
                  </div>
                  <button 
                    id="new-analysis-button"
                    onClick={onStartNewAnalysis}
                    className="relative bg-primary-blue text-white font-bold py-2 px-5 rounded-lg transition-all duration-200 inline-flex items-center shadow-sm hover:shadow-md"
                  >
              ...
              <div>
                  <div className="flex justify-between items-center mt-6 mb-4">
                    <h2 className="text-2xl font-bold text-primary-text-light dark:text-primary-text-dark">Analyses</h2>
                    <div className="flex items-center border border-border-light dark:border-border-dark rounded-lg p-1 bg-light-card dark:bg-dark-card">
                        <button onClick={() => setView('active')} className={`px-4 py-1.5 rounded-md text-sm font-semibold transition-colors ${view === 'active' ? 'bg-primary-blue text-white' : 'text-secondary-text-light dark:text-secondary-text-dark'}`}>Active ({activeReports.length})</button>
                        <button onClick={() => setView('archived')} className={`px-4 py-1.5 rounded-md text-sm font-semibold transition-colors ${view === 'archived' ? 'bg-primary-blue text-white' : 'text-secondary-text-light dark:text-secondary-text-dark'}`}>Archived ({archivedReports.length})</button>
                    </div>
                  </div>
                  ...
                  {reportsToShow.length > 0 ? (
                    ...
                    <div className="text-center bg-light-card dark:bg-dark-card p-12 rounded-xl border border-border-light dark:border-border-dark">
                        <h2 className="text-2xl font-bold text-primary-text-light dark:text-primary-text-dark">No {view} analyses</h2>
