      ...
      <div className="p-8 space-y-6">
        <div className="bg-vesta-card-light dark:bg-vesta-card-dark p-4 rounded-lg shadow-md border border-vesta-border-light dark:border-vesta-border-dark flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex flex-col md:flex-row items-center gap-4 w-full">
                ...
                />
                <select className="w-full md:w-64 px-4 py-2 border border-vesta-border-light dark:border-vesta-border-dark rounded-lg focus:outline-none focus:ring-2 focus:ring-vesta-red bg-vesta-bg-light dark:bg-vesta-bg-dark text-vesta-text-light dark:text-vesta-text-dark appearance-none bg-no-repeat" style={{backgroundImage: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%236b7280' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e")`, backgroundPosition: 'right 0.5rem center', backgroundSize: '1.5em 1.5em'}}>
        ...
        <div className="bg-vesta-card-light dark:bg-vesta-card-dark rounded-lg shadow-md overflow-hidden border border-vesta-border-light dark:border-vesta-border-dark">
          {logs.length > 0 ? (
                  ...
                  <tbody>
                    {logs.map((log) => (
                      <tr key={log.id} className="border-b border-vesta-border-light dark:border-vesta-border-dark last:border-b-0 hover:bg-gray-50 dark:hover:bg-vesta-bg-dark/20 transition-colors">
                        ...
                        <td className="p-4 text-vesta-text-light dark:text-vesta-text-dark max-w-lg truncate">{log.details}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="p-12 text-center">
                ...
                <p className="mt-1 text-vesta-text-secondary-light dark:text-vesta-text-secondary-dark">Your activity, such as document uploads and analyses, will be logged here.</p>
            </div>
          )}
        </div>
      </div>
